...
public:
	...
	std::pair<int32_t, int32_t> wait_for_sum(const int32_t &sum);
	std::future<std::pair<int32_t, int32_t>> wait_for_sum_(const int32_t &sum);

private:
	std::map<size_t, std::promise<int32_t>> pending_requests_plus;
	std::map<size_t, std::promise<double>> pending_requests_divide;
	std::map<size_t, std::promise<std::pair<int32_t, int32_t>>> pending_requests_wait_for_sum;
