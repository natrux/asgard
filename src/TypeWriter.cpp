...
void TypeWriter::write_typecode(const Typecode &type){
...
void TypeWriter::write_signature(const Signature &signature){
	...
	if(!written){
		...
		const length_t num_parents = signature.parents.size();
		write_value(num_parents);
		for(const auto &parent : signature.parents){
			write_value(parent);
		}

		const length_t num_members = signature.members.size();
...
void TypeWriter::write_enum_map(const EnumMap &map){
	...
	if(!written){
		...
		const length_t num_values = map.enum_map.size();
...
void TypeWriter::write_empty_value(const Typecode &type){
	switch(type.code){
	...
	case Typecode::TYPE_I64:
		...
		break;
	case Typecode::TYPE_F32:
		write_value(static_cast<float>(0));
		break;
	case Typecode::TYPE_F64:
	...
	case Typecode::TYPE_ENUM:
		write_enum_map(EnumMap());
		write_value(static_cast<enum_t>(0));
		break;
	default:
		break;
	}
